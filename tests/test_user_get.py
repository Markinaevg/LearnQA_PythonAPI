...
#Получение данных пользователя никем не авторизованного
...
#Авторизация и запрос на одного и того же пользователя
...
# И убедиться, что в этом случае запрос также получает только username, так как мы не должны видеть остальные данные чужого пользователя.
    def test_get_user_details_auth_of_another_user(self):
        ...
        auth_sid = self.get_cookie(response1, "auth_sid")
        token = self.get_header(response1, "x-csrf-token")
        second_user_id = self.get_json_value(response1, "user_id") - 1
